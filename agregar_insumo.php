...
if (isset($_GET['buscar_insumo'])) {
    ...
    $sugerencias = [];
    while ($row = $resultado->fetch_assoc()) { $sugerencias[] = $row; }
    echo json_encode($sugerencias);
    $conn->close();
    exit;
}

// --- 2. PROCESAR GUARDADO (Solo actualización de inventario) ---
$mensaje = "";
$tipo_mensaje = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // A. Obtener o crear insumo en catálogo
        $check = $conn->query("SELECT id FROM insumos WHERE LOWER(nombre) = LOWER('$nombre') LIMIT 1");
        
        if ($check->num_rows > 0) {
            $insumo = $check->fetch_assoc();
            $insumo_id = $insumo['id'];
        } else {
            $uni = $conn->real_escape_string($_POST['unidad_medida']);
            $conn->query("INSERT INTO insumos (nombre, unidad_medida) VALUES ('$nombre', '$uni')");
            $insumo_id = $conn->insert_id;
        }

        // B. Actualizar stock directamente en inventario
        $sql_inventario = "INSERT INTO inventario (centro_id, insumo_id, cantidad, ultima_actualizacion) 
                           VALUES ($centro_usuario, $insumo_id, $cantidad_ingresar, NOW())
                           ON DUPLICATE KEY UPDATE 
                           cantidad = cantidad + $cantidad_ingresar, 
...
<div class="max-w-xl mx-auto py-6 px-4 sm:px-6" 
     x-data="{
        ...
        cargando: false,
        mostrarLista: false,
        insumoData: { cantidad: 0, unidad_medida: '' },
        ...
        limpiarSeleccion() {
            ...
            this.existe = false;
            this.sugerencias = [];
            this.insumoData = { cantidad: 0, unidad_medida: '' };
        }
     }"
            ...
            <form action="agregar_insumo.php" method="POST" class="space-y-4">
                ...
                <div x-show="existe" x-cloak 
                    ...
                    <div class="text-xs">
                        ...
                        <p class="mt-0.5 text-amber-700">Stock actual: <strong x-text="insumoData.cantidad + ' ' + insumoData.unidad_medida"></strong>. La cantidad ingresada se **sumará** al stock existente en tu centro.</p>
                    </div>
                </div>
                ...
                <div x-show="!existe && nombreInsumo.trim().length >= 2 && !cargando && sugerencias.length === 0" x-cloak 
                    ...
                    <div class="text-xs">
                        ...
                        <p class="mt-0.5 text-sky-700">Puedes continuar escribiendo el nombre completo y rellenar la unidad de medida de abajo para guardarlo por primera vez.</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 gap-4" x-show="!existe" x-transition>
